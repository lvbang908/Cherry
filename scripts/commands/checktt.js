...
module.exports.run = async function ({ args, api, event, Threads, Users, Cherry }) {
	...
	if (args[0] == "all") {
        ...
        for (const lastData of allInfo) {
            ...
            number++;
            msg += `Hạng ${number}. ${userInfo = "" || !userInfo || userInfo == undefined ? lastData.ID : userInfo.name} với ${lastData.totalMsg} tin nhắn \n`;
        }	
		...
		for (const lastData of allInfo) {
			...
			for (var i of mention) {
				if (lastData.ID == i) {
					msg += `${userInfo = "" || !userInfo || userInfo == undefined ? lastData.ID : userInfo.name} đứng hạng ${number} với ${lastData.totalMsg} tin nhắn \n`;
				}
			}
        }
    ...
    Cherry.error(this.config.name, threadID, messageID, api, Threads);
}