...
module.exports.handleMessageReply = async function({ api, event, Reply }) {
	if (isNaN(event.body)) return api.sendMessage('Bạn phải nhập số tương đương với bài muốn chọn!', event.threadID, event.messageID);
	...
	api.sendMessage("Ngồi đó chờ tao 5 phút để tao đi lấy video về cho mày :/", event.threadID, (error, info) => { Cherry.autoUnsend(info.messageID, 15000, api) }, event.messageID);
	try {
