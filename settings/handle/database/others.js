...
module.exports = function ({ Cherry }) {
    ...
    async function createData(userID, callback) {
        try {
            if (JSON.stringify(othersData) === "{}" || !othersData) {
                othersData = {
                    [userID]: {
                        ID: userID,
                        workTime: 0,
            ...
            log("OTHERS - CREATE DATA", error.stack, "error");
        }
    }
    ...
    async function getAll(keys, callback) {
        try {
            ...
            log("OTHERS - GET ALL DATA", error.stack, "error");
            if (callback && typeof callback == "function") callback(error, null);
        }
    }
    ...
    async function getData(userID, callback) {
        try {
            ...
            if (JSON.stringify(othersData) === "{}" || !othersData[userID]) await createData(userID);
            const data = othersData[userID];
            if (callback && typeof callback == "function") callback(null, data);
            return data;
        }
        catch(error) {
            log("OTHERS - GET DATA", error.stack, "error");
            if (callback && typeof callback == "function") callback(error, null);
        }
    }
    ...
    async function setData(userID, options, callback) {
        try {
            ...
            log("OTHERS - SET DATA", error, "error");
            if (callback && typeof callback == "function") callback(error, null);
        }
    }
    ...
    async function delData(userID, callback) {
        try {
            ...
            let dataOthers = JSON.stringify(othersData, null, 4);
            await fs.writeFileSync("./settings/data/othersData.json", dataOthers);
            if (callback && typeof callback == "function") callback(null, data);
        } catch (error) {
            log("DELDATA OTHERS", error.stack, "error");
            if (callback && typeof callback == "function") callback(error, null);
        }
    }
    return {
